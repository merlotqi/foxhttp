...
{
    if (enable_gzip)
        supported_types_.push_back(compression_type::gzip);
    if (enable_deflate)
        supported_types_.push_back(compression_type::deflate);
    if (enable_br)
        supported_types_.push_back(compression_type::br);
}

std::string compression_middleware::name() const
{
...
bool compression_middleware::_should_compress(const request_context &ctx, const http::response<http::string_body> &res)
{
