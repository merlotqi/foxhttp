...
{
    return response_.result() == http::status::ok;
}

bool response::is_successful() const
{
    return response_.result_int() >= 200 && response_.result_int() < 300;
}
...
{
    return response_.find(name) != response_.end();
}
...
const http::response<http::string_body> &response::native() const
{
    return response_;
}

http::response<http::string_body> &response::native()
{
    return response_;
}
...
const http::request<http::string_body> &request::native() const
{
    return request_;
}

http::request<http::string_body> &request::native()
{
    return request_;
}
