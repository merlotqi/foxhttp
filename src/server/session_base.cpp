...
session_base::session_base(const boost::asio::any_io_executor &executor, std::shared_ptr<middleware_chain> chain,
    ...
    : executor_(executor), chain_(std::move(chain)), limits_(limits), idle_timer_(executor_), header_timer_(executor_),
      body_timer_(executor_)
{
}

void session_base::set_limits(const session_limits &limits)
{
    limits_ = limits;
}

const session_limits &session_base::limits() const
{
    return limits_;
}
...
void session_base::cancel_idle_timer()
{
    idle_timer_.cancel();
}
...
void session_base::cancel_header_timer()
{
...
void session_base::cancel_body_timer()
{
    body_timer_.cancel();
}
