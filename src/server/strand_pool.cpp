...
void printStrandPoolStatistics(const strand_pool &pool)
{
    ...
    for (std::size_t i = 0; i < stats.size(); ++i)
    {
        ...
        std::cout << "    Avg Execution Time: " << formatDuration(stat.average_execution_time()) << std::endl;
...
strand_pool::strand_pool(boost::asio::io_context &io_context, const strand_pool_config &config)
    : io_context_(io_context), config_(config), current_size_(0), round_robin_index_(0),
      random_generator_(std::random_device{}()), metrics_timer_(io_context), health_check_timer_(io_context),
      is_running_(false)
{
    _initialize();
}
...
strand_pool::strand_ptr strand_pool::strand_by_hash(std::size_t hash)
{
    return _consistent_hash_strand(hash);
}
...
void strand_pool::set_load_balance_strategy(load_balance_strategy strategy)
{
...
const strand_pool_config &strand_pool::config() const
{
    return config_;
}
...
void strand_pool::perform_health_check()
{
    _perform_health_check_internal();
}

void strand_pool::report_metrics()
{
...
void strand_pool::_report_metrics_internal()
{
    ...
    for (std::size_t i = 0; i < current_size_; ++i)
    {
        ...
        metrics_callback_("strand_active_requests", std::to_string(i),
                          static_cast<double>(stats.active_requests.load()));
