...
bool timer_task::is_expired(const time_point_t &now) const { return expiry_time <= now; }

time_point_t timer_task::next_execution_time() const { return expiry_time + interval; }
...
timer_bucket::~timer_bucket() { stop(); }
...
const timer_manager_config &timer_manager::config() const { return config_; }

// 更新配置
void timer_manager::update_config(const timer_manager_config &new_config) { config_ = new_config; }
