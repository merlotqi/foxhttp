...
void ws_session::start_accept(http::request<http::string_body> req) {
  ...
  ws_.async_accept(req, [self = shared_from_this()](boost::system::error_code ec) {
    static_cast<ws_session *>(self.get())->_on_accept(ec);
  });
}

void ws_session::_on_accept(boost::system::error_code ec) {
  if (ec) return;
  _do_read();
}

void ws_session::_do_read() {
  ws_.async_read(buffer_, [self = shared_from_this()](boost::system::error_code ec, std::size_t bytes_transferred) {
    static_cast<ws_session *>(self.get())->_on_read(ec, bytes_transferred);
  });
}

void ws_session::_on_read(boost::system::error_code ec, std::size_t) {
  if (ec) return;
  // Echo
