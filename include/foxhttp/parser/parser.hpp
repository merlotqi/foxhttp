...
class ParserHolderBase {
 public:
  virtual ~ParserHolderBase() = default;
  virtual std::string name() const = 0;
  virtual std::string content_type() const = 0;
  virtual bool supports(const http::request<http::string_body> &req) const = 0;
};
...
template <typename T>
class ParserHolder : public ParserHolderBase {
 public:
  explicit ParserHolder(std::shared_ptr<Parser<T>> impl) : impl_(std::move(impl)) {}
...
class ParserFactory {
 public:
  ...
  void register_parser(std::shared_ptr<Parser<T>> parser) {
    ...
    vec.push_back(std::make_shared<ParserHolder<T>>(std::move(parser)));
  }
  ...
  std::shared_ptr<Parser<T>> get_best(const http::request<http::string_body> &req) const {
    ...
    for (auto &holder_base : it->second) {
      auto holder = std::dynamic_pointer_cast<ParserHolder<T>>(holder_base);
 ...
 private:
  std::unordered_map<std::type_index, std::vector<std::shared_ptr<ParserHolderBase>>> registry_;
};
}  // namespace foxhttp::parser
...
#define REGISTER_PARSER(TYPE, CLASSNAME)                                                        \
  ...
  struct CLASSNAME##Registrar {                                                                 \
    CLASSNAME##Registrar() {                                                                    \
      foxhttp::parser::ParserFactory::instance().register_parser<TYPE>(std::make_shared<CLASSNAME>()); \
    }                                                                                           \
  };                                                                                            \
