...
public:
    ...
    using basic_validator = std::function<std::optional<auth_user>(const std::string &username,
...
public:
    explicit auth_middleware(std::vector<auth_scheme> schemes)
            : schemes_(std::move(schemes)) {}

    // Builder-style configuration
    auth_middleware &set_jwt_validator(jwt_validator v) { jwt_validator_ = std::move(v); return *this; }
    auth_middleware &set_basic_validator(basic_validator v) { basic_validator_ = std::move(v); return *this; }
    auth_middleware &set_digest_validator(digest_validator v) { digest_validator_ = std::move(v); return *this; }
    auth_middleware &set_api_key_validator(api_key_validator v) { api_key_validator_ = std::move(v); return *this; }
    auth_middleware &set_oauth2_validator(oauth2_validator v) { oauth2_validator_ = std::move(v); return *this; }
    auth_middleware &set_session_lookup(session_lookup v) { session_lookup_ = std::move(v); return *this; }

    auth_middleware &set_api_key_header(std::string name) { api_key_header_ = std::move(name); return *this; }
    auth_middleware &set_api_key_query_param(std::string name) { api_key_query_param_ = std::move(name); return *this; }
    auth_middleware &set_session_cookie_name(std::string name) { session_cookie_name_ = std::move(name); return *this; }
    auth_middleware &set_realm(std::string realm) { realm_ = std::move(realm); return *this; }
...
private:
    ...
    static std::unordered_map<std::string, std::string> _parse_kv(const std::string &s)
    {
        ...
        for (size_t i = 0; i < s.size(); ++i)
        {
            ...
            if (state == K)
            {
                if (c == '=') state = V; else if (std::isalnum(static_cast<unsigned char>(c)) || c=='-'||c=='_') key.push_back(c);
            }
            else if (state == V)
            {
                if (c == '"') { state = Q; }
                else if (c == ',') { out[key] = val; key.clear(); val.clear(); state = K; }
                else if (!std::iscntrl(static_cast<unsigned char>(c))) val.push_back(c);
            }
            else if (state == Q)
            {
                if (c == '"') { state = V; }
